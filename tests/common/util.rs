...
/// * [`UCommand::with_util`]: Run `procps UTIL_NAME` instead of the shell
/// * [`UCommand::from_test_scenario`]: Run `procps UTIL_NAME` instead of the shell in the
...
impl UCommand {
    ...
    /// execution binary is set to `procps`.
...
pub fn host_name_for(util_name: &str) -> Cow<str> {
    // In some environments, e.g. macOS/freebsd, the GNU procps are prefixed with "g"
